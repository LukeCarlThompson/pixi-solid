...
<Aside type="caution">
  When using `existingApp`, you are responsible for:
  <ul>
    <li>Ensuring the application is initialized before rendering the provider</li>
    <li>
...
```tsx
import type { PixiApplicationProps } from "pixi-solid";
```
...
- `createDelay()` - Create ticker-synced callback delays
...
- [`PixiCanvas`](../pixi-canvas) - For rendering the actual canvas to the DOM
- [`TickerProvider`](../ticker-provider) - For ticker functionality without a full application
