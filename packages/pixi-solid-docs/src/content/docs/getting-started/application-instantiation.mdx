...
<Aside type="tip">
  When using `PixiApplicationProvider`, any HTML components placed between the provider and the
  canvas can access pixi-solid hooks and the Pixi application context. This allows you to build UIs
  that react to Pixi app state, like displaying canvas dimensions or rendering status.
</Aside>
