...
description: A Solid.js function for creating an async delay function in sync with the Pixi.js Ticker.
sidebar:
  order: 4
---

The `createAsyncDelay` function provides a way to create an async function that will delay execution for a specified number of milliseconds of ticker time.
...
This function must be called within a SolidJS tracking scope, such as a component or another function, that is a descendant of `PixiApplicationProvider` or `PixiCanvas`.
