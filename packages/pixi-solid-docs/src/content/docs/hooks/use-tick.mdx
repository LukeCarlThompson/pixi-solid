---
title: useTick
description: Solid.js hooks for subscribing and unsubscribing to the Pixi.js Ticker.
sidebar:
  order: 3
---
...
The `useTick` hook is a more focused utility for registering a callback function to be executed on every frame of the `PIXI.Application`'s ticker. The callback is automatically added when the component mounts and removed when the component or hook's owning computation is cleaned up, ensuring proper resource management.
...
```tsx
import { useTick, Sprite } from "pixi-solid";
