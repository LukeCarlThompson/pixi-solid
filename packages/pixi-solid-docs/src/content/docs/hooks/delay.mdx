...
<Aside type="note">
  This function cannot be used in an async context because the Application and Ticker context
  providers aren't available in an async context. For that purpose you should use the alternative
  `createAsyncDelay` hook.
</Aside>
