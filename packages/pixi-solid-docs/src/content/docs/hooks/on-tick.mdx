...
It must be called within a SolidJS tracking scope, such as a component or another hook, that is a descendant of either `<PixiApplicationProvider />` or `<PixiCanvas />`.
