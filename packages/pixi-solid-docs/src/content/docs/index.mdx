...
- **Declerative PixiJS scene graph**: Using SolidJS's JSX templating means we get declerative control over the scene graph. No longer necessary to imperatively add and remove children.
...
- **SolidJS is really fast**: SolidJs is on of the fatsest front0end frameworks out there so the overhead is very minimal.

- **SolidJS is fully featured**: It has stores, signals, suspense, error boundaries, resource fetching and more. It's a great feature set for simple of complex applications and you won't have to reach for other libraries to manage templating or state.
